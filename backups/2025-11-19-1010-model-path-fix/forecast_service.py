...
QUICK WIN #1: ElasticNet as PRIMARY, LightGBM as BACKUP
"""
import json
import random
...
class ForecastService:
    ...
    def _load_ml_model(self, horizon: str) -> Optional[Dict]:
        ...
        PRIORITY: ElasticNet (primary) -> LightGBM (fallback)
        """
        ...
        try:
            ...
            # Path base based on environment
            if self.development_mode:
                base_path = Path("/Users/rafaelfarias/Documents/Recursos/Proyectos/forex-forecast-systemV2/models/trained")
            else:
                base_path = Path("/app/models/trained")
            ...
            if elasticnet_path.exists():
                ...
                model_dict = joblib.load(elasticnet_path)
                model_dict['model_type'] = 'ElasticNet'
            ...
            if lightgbm_path.exists():
                print(f"⚠ Fallback to LightGBM BACKUP model for {horizon}")
                model_dict = joblib.load(lightgbm_path)
                model_dict['model_type'] = 'LightGBM'
                self._models_cache[horizon] = model_dict
                return model_dict

            print(f"✗ No model found for {horizon} (tried ElasticNet + LightGBM)")
            return None
    ...
    async def get_forecast(self, horizon: str) -> Optional[ForecastResponse]:
        ...
        if model_dict:
            ...
            model_name = model_dict.get('model_type', 'Unknown')
            print(f"Using {model_name} for {horizon}: MAPE={mape:.2f}%, Accuracy={accuracy_score:.2%}")
        else:
