"""
Command-line interface for 12-month forecaster service.
...
from .pipeline import run_forecast_pipeline, validate_forecast, _resample_to_monthly

app = typer.Typer(
    name="forecaster-12m",
    help="12-month USD/CLP forex forecasting service",
...
def run(
    ...
    log_file: Optional[Path] = typer.Option(
        None,
        "--log-file",
        help="Path to log file (default: logs/forecaster_12m.log)",
    ),
) -> None:
    """
    Run the 12-month forecast pipeline.
    ...
    Examples:
        ...
        $ python -m services.forecaster_12m.cli run -o ./my_reports -l DEBUG
    """
    # Configure logging
    if log_file is None:
        log_file = Path("./logs/forecaster_12m.log")

    configure_logging(log_path=log_file, level=log_level)

    console.print("\n[bold cyan]12-Month Forex Forecaster[/bold cyan]")
    console.print("=" * 60)
...
def validate(
    ...
    Examples:
        ...
        $ python -m services.forecaster_12m.cli validate -l DEBUG
    """
    configure_logging(level=log_level)

    console.print("\n[bold cyan]Validating 12-Month Forecast[/bold cyan]")
    console.print("=" * 60)
    ...
    try:
        ...
        # Resample to monthly
        with console.status("[yellow]Resampling to monthly...[/yellow]"):
            bundle = _resample_to_monthly(bundle)

        console.print(f"[green]✓ Data resampled:[/green] Monthly frequency")
        ...
        if is_valid:
            ...
            table = Table(title="12-Month Forecast Summary")
...
def backtest(
    ...
    Examples:
        $ python -m services.forecaster_12m.cli backtest --months 12
        $ python -m services.forecaster_12m.cli backtest -m 24 -o ./backtest_results
    """
    configure_logging(level=log_level)

    console.print("\n[bold cyan]Backtesting 12-Month Forecasts[/bold cyan]")
    console.print("=" * 60)
    console.print(f"[yellow]Months to backtest:[/yellow] {months}")
...
def info() -> None:
    ...
    console.print("\n[bold cyan]Forecaster 12M - Configuration[/bold cyan]")
    console.print("=" * 60)
    ...
    table.add_row("Projection Months", str(service_config.projection_months))
