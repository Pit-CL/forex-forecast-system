"""
Command-line interface for 30-day forecaster service.
...
app = typer.Typer(
    name="forecaster-30d",
    help="30-day USD/CLP forex forecasting service",
...
def run(
    ...
    log_file: Optional[Path] = typer.Option(
        None,
        "--log-file",
        help="Path to log file (default: logs/forecaster_30d.log)",
    ),
) -> None:
    """
    Run the 30-day forecast pipeline.

    This command executes the complete forecasting workflow:
    - Load data from multiple sources
    - Generate 30-day ensemble forecast
    ...
    Examples:
        ...
        $ python -m services.forecaster_30d.cli run

        # Run without email
        $ python -m services.forecaster_30d.cli run --skip-email

        # Custom output directory and debug logging
        $ python -m services.forecaster_30d.cli run -o ./my_reports -l DEBUG
    """
    # Configure logging
    if log_file is None:
        log_file = Path("./logs/forecaster_30d.log")
...
def validate(
    ...
    Examples:
        $ python -m services.forecaster_30d.cli validate
        $ python -m services.forecaster_30d.cli validate -l DEBUG
    """
...
def backtest(
    ...
    Examples:
        $ python -m services.forecaster_30d.cli backtest --days 30
        $ python -m services.forecaster_30d.cli backtest -d 60 -o ./backtest_results
    """
    configure_logging(level=log_level)
...
def info() -> None:
    ...
    console.print("\n[bold cyan]Forecaster 30D - Configuration[/bold cyan]")
    console.print("=" * 60)
