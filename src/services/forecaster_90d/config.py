"""
Configuration overrides for 90-day forecaster service.

This module provides service-specific configuration that extends
the base forex_core settings with 90-day forecast parameters.
"""

from __future__ import annotations

from dataclasses import dataclass
from typing import Literal

from forex_core.config import (
    PROJECTION_DAYS_90D,
    HISTORICAL_LOOKBACK_DAYS_90D,
    TECH_LOOKBACK_DAYS_90D,
    VOL_LOOKBACK_DAYS_90D,
)


@dataclass(frozen=True)
class Forecaster90DConfig:
    """
    Service-specific configuration for 90-day forecaster.

    These settings override or extend the base forex_core configuration
    for quarterly forecasting (executed on day 1 of each month).

    Attributes:
        horizon: Forecast horizon type (always "daily" for this service).
        projection_days: Number of days to forecast (90).
        frequency: Pandas frequency string for daily data.
        historical_lookback_days: Days of historical data to use.
        tech_lookback_days: Days for technical indicator calculation.
        vol_lookback_days: Days for volatility calculation.
        report_title: Default title for generated reports.
        report_filename_prefix: Prefix for output filenames.

    Example:
        >>> config = Forecaster90DConfig()
        >>> print(config.projection_days)
        90
        >>> print(config.horizon)
        'daily'
    """

    # Forecast horizon
    horizon: Literal["daily"] = "daily"
    projection_days: int = PROJECTION_DAYS_90D  # 90 days
    frequency: str = "D"  # Daily frequency

    # Lookback periods (recommended by usdclp agent)
    historical_lookback_days: int = HISTORICAL_LOOKBACK_DAYS_90D  # 1095 days (3 years)
    tech_lookback_days: int = TECH_LOOKBACK_DAYS_90D  # 180 days (~6 months)
    vol_lookback_days: int = VOL_LOOKBACK_DAYS_90D  # 120 days (4 months)

    # Report configuration
    report_title: str = "Proyección USD/CLP - Próximos 90 Días"
    report_filename_prefix: str = "Forecast_90D_USDCLP"

    # Chart configuration
    chart_title_suffix: str = "(90 días / trimestral)"

    @property
    def steps(self) -> int:
        """Number of forecast steps (same as projection_days for daily)."""
        return self.projection_days

    @property
    def horizon_code(self) -> str:
        """Horizon code for reporting and interpretations (e.g., '90d')."""
        return "90d"


def get_service_config() -> Forecaster90DConfig:
    """
    Get the service-specific configuration.

    Returns:
        Frozen configuration instance for 90-day forecaster.

    Example:
        >>> config = get_service_config()
        >>> assert config.horizon == "daily"
    """
    return Forecaster90DConfig()


__all__ = ["Forecaster90DConfig", "get_service_config"]
