"""
Command-line interface for 15-day forecaster service.
...
app = typer.Typer(
    name="forecaster-15d",
    help="15-day USD/CLP forex forecasting service",
...
def run(
    ...
    log_file: Optional[Path] = typer.Option(
        None,
        "--log-file",
        help="Path to log file (default: logs/forecaster_15d.log)",
    ),
) -> None:
    """
    Run the 15-day forecast pipeline.

    This command executes the complete forecasting workflow:
    - Load data from multiple sources
    - Generate 15-day ensemble forecast
    ...
    Examples:
        ...
        $ python -m services.forecaster_15d.cli run

        # Run without email
        $ python -m services.forecaster_15d.cli run --skip-email

        # Custom output directory and debug logging
        $ python -m services.forecaster_15d.cli run -o ./my_reports -l DEBUG
    """
    # Configure logging
    if log_file is None:
        log_file = Path("./logs/forecaster_15d.log")
...
def backtest(
    ...
    Examples:
        $ python -m services.forecaster_15d.cli backtest --days 30
        $ python -m services.forecaster_15d.cli backtest -d 60 -o ./backtest_results
    """
    configure_logging(level=log_level)
...
def info() -> None:
    ...
    console.print("\n[bold cyan]Forecaster 15D - Configuration[/bold cyan]")
    console.print("=" * 60)
