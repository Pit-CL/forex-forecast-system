...
def load_latest_data(data_path: Path) -> pd.DataFrame:
    ...
    fred_path = data_path / "raw" / "fred_interest_rates.csv"
    if fred_path.exists():
        ...
        print(f"Merged FRED: rate_differential available")
...
def compute_features(df: pd.DataFrame, feature_names: list) -> pd.DataFrame:
    ...
    # FRED interest rate differential (NEW 2025-11-22)
    if 'rate_differential' in df.columns:
        features_df['rate_differential'] = df['rate_differential']
...
def select_best_model(models_path: Path, horizon_upper: str) -> tuple:
    ...
    if elasticnet_path.exists():
        try:
            ...
            print(f"⚠️  Could not load ElasticNet: {e}")
    ...
    print(f"\n🏆 MODEL SELECTION for {horizon_upper}:")
...
def apply_economic_bounds(forecast_dict: dict, horizon_days: int) -> dict:
    ...
    if bounded_forecast != raw_forecast:
        ...
        print(f"⚠️  Economic bound applied {horizon_days}D: "
              f"${raw_forecast:.2f} ({change_pct:+.2f}%) → "
...
def main():
    ...
    for horizon_str in horizons:
        ...
        try:
            ...
            print(f"⚠️  {model_name} failed (incompatible features)")
            ...
            if elasticnet_path.exists() and model_name != "ElasticNet":
                try:
                    ...
                    forecast_dict['model_selection'] = comparison_info
                    forecast_successful = True
                    print(f"✅ Using ElasticNet fallback")

                except Exception as e2:
                    print(f"✗ Fallback failed: {e2}")

        if not forecast_successful:
            print(f"✗ All models failed for {horizon_upper}")
            continue
