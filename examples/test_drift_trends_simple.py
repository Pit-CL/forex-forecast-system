"""
Simple test for DriftTrendAnalyzer using cached warehouse data.

This script tests the drift trend analysis system by:
1. Loading cached USD/CLP data from warehouse
...
def test_drift_trends_simple():
    """Test drift trend analysis with cached warehouse data."""
    logger.info("=" * 60)
    logger.info("Testing DriftTrendAnalyzer (Simple)")
    logger.info("=" * 60)

    # Load cached USD/CLP data directly from warehouse
    logger.info("\n1. Loading cached USD/CLP data from warehouse...")
    ...
    if not warehouse_path.exists():
        ...
        logger.info("Run a forecast first to populate warehouse data")
        return

    try:
        df = pd.read_parquet(warehouse_path)
        series = df["value"]  # Index is already the date
    ...
    if len(series) < lookback + 120:  # Need baseline + lookback
        ...
        logger.info(f"Need at least {lookback + 120} points")
        return
    ...
    drift_count = 0
    logger.info(f"Will check {lookback // step} time windows...")
